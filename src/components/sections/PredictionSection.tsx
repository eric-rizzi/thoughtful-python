...
import type { PredictionSection as PredictionSectionData, PredictionTableRow } from '../../types/data';
...
const PredictionSection: React.FC<PredictionSectionProps> = ({
  ...
  const checkIfAllCorrect = (currentPredictions: PredictionState, rows: PredictionTableRow[]) => {
      const allDone = rows.every((row, index) =>
          currentPredictions[index]?.isCorrect === true
      );
  ...
  const handlePredictionChange = useCallback((rowIndex: number, newValue: string) => {
    ...
    const allAreCorrect = checkIfAllCorrect(updatedPredictions, section.predictionTable.rows);
    saveProgress<SavedPredictionState>(storageKey, { predictions: updatedPredictions });

  }, [predictions, section.predictionTable.rows, onSectionComplete, section.id, storageKey]); // Dependencies
    ...
    <section id={section.id} className={styles.section}>
      ...
      {section.functionDisplay && (
        <div className={styles.functionDisplayContainer}>
          <h4 className={styles.functionDisplayTitle}>{section.functionDisplay.title}</h4>
        ...
        <table className={styles.predictionTable}>
          ...
          <tbody>
            {section.predictionTable.rows.map((row, rowIndex) => {
              ...
              const rowClass = rowState?.isCorrect === true ? styles.correctRow :
                               rowState?.isCorrect === false ? styles.incorrectRow : '';
              const inputClass = rowState?.isCorrect === true ? styles.predictionInputCorrect :
                                 rowState?.isCorrect === false ? styles.predictionInputIncorrect :
              ...
              return (
                <tr key={rowIndex} className={rowClass}>
                  ...
                  <td className={styles.statusCell}>
                    {rowState?.isCorrect === true && <span className={styles.statusIndicatorCorrect}></span>}
                    {rowState?.isCorrect === false && <span className={styles.statusIndicatorIncorrect}></span>}
