...
async function getSimulatedFeedback(
  ...
  const topicKeywords: { [key: string]: string[] } = {
    ...
    datastructures: ['list', 'dictionary', 'set', 'tuple', 'append', 'key', 'value'],
    turtle: ['turtle', 'forward', 'left', 'right', 'penup', 'pendown']
  };

  const relevantKeywords = topicKeywords[submission.topic] || [];
  const keywordMatch = relevantKeywords.some(kw => codeLower.includes(kw) || explanationLower.includes(kw));

  if (submission.code.length < 20 || submission.explanation.length < 40) {
    assessment = 'developing';
    feedback = rubric?.developing || "Your code example or explanation seems a bit brief. Try elaborating more on the concept and how your code demonstrates it.";
  } else if (submission.explanation.length > 100 && keywordMatch && submission.code.length > 30) {
    assessment = 'exceeds';
    feedback = rubric?.exceeds || "Excellent! Your code is clear, and your explanation thoroughly demonstrates a strong understanding of the topic. You've connected the code well to the concept.";
  } else {
    assessment = 'meets';
    feedback = rubric?.meets || "Good job! Your code example works and your explanation covers the main points of the topic.";
  }
...
const ReflectionSection: React.FC<ReflectionSectionProps> = ({
  ...
  useEffect(() => {
    ...
    if (savedState?.history) {
      ...
      const alreadyComplete = savedState.history.some(entry =>
        entry.response?.assessment && ['meets', 'exceeds'].includes(entry.response.assessment)
      );
      if (alreadyComplete) {
  ...
  const handleSubmit = useCallback(async (isFormalSubmission: boolean) => {
    // Basic validation
    if (!topic || !code.trim() || !explanation.trim()) {
      alert('Please select a topic, write some code, and provide an explanation.');
      return;
    }
    ...
    try {
      ...
      const newHistoryEntry: ReflectionHistoryEntry = { submission, response };
  ...
  }, [topic, code, explanation, section.rubric, section.id, history, onSectionComplete, storageKey]); // Added history dependency

  // Helper to get topic display name
  const getTopicName = (topicValue: string): string => {
    const optionElement = document.querySelector(`#${section.id}-topic option[value="${topicValue}"]`);
    ...
    <section id={section.id} className={styles.section}>
      ...
      <div className={styles.reflectionContainer}>
        ...
        <div className={styles.reflectionInputGroup}>
          <label htmlFor={`${section.id}-topic`} className={styles.reflectionLabel}>
        ...
        <div className={styles.reflectionInputGroup}>
            <label htmlFor={`${section.id}-explanation`} className={styles.reflectionLabel}>
        ...
        <div className={styles.reflectionButtons}>
            <button
                ...
                disabled={isSubmitting || !topic || !code.trim() || !explanation.trim()}
            ...
            {history.length === 0 ? (
                ...
                history.map((entry, index) => (
                   <div
                        ...
                        className={`${styles.reflectionCard} ${entry.response?.assessment ? styles[`cardAssessment${entry.response.assessment.charAt(0).toUpperCase() + entry.response.assessment.slice(1)}`] : ''}`}
                    >
                        <div className={styles.reflectionSubmission}>
                           <div className={styles.reflectionHeader}>
                                <span className={styles.reflectionDate}>{formatDate(entry.submission.timestamp)}</span>
                                {entry.submission.submitted && <span className={styles.submissionBadge}>Submitted to Journal</span>}
                            </div>
                            <h5>Topic: {getTopicName(entry.submission.topic)}</h5>
                            <details> {/* Use details/summary for collapsibility */}
                                <summary style={{cursor: 'pointer', fontWeight: '500'}}>Show Code & Explanation</summary>
                                <div className={styles.reflectionCodeDisplay}>
                                    <pre><code>{entry.submission.code}</code></pre>
                                </div>
                        ...
                        {entry.response && (
                             <div className={styles.reflectionResponse}>
                                ...
                                <div className={styles[`assessmentBadge${entry.response.assessment.charAt(0).toUpperCase() + entry.response.assessment.slice(1)}`]}>
                                    {entry.response.assessment}
                                </div>
                                <p>{entry.response.feedback}</p>
                             </div>
                        )}
                        {!entry.response && isSubmitting && index === 0 && ( // Show loading only for the latest if still submitting
                             <div className={styles.reflectionResponse}>
                                 <p><i>Getting feedback...</i></p>
                             </div>
                        )}
                   </div>
                ))
